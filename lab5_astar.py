...
def a_star(fringe,goal):
    ...
    while not fringe.empty():
        ...
        #print expand_node.state, "expand node"
        ...
        if expand_node.state not in closed:
            ...
            print "expand_node goal", expand_node.state, goal
            #Test for the goal
...
def get_neighbors(g, closed):
    ...
    for i in range(g[0]-1, g[0]+2):
        if ((i < dmap.grid_width) and (i > 0)):
            #print "first for first iff"
            for j in range(g[1]-1, g[1]+2):
                #print i, j, "i and j: ", dmap.grid_height, dmap.grid_width, "height and width"
                if ((j < dmap.grid_height) and (j > 0)):
                    #print "second iff"
                    if (((i,j) not in closed) and ((i,j) not in dmap.occupied)):
