...
def goal_force( ):
    ...
    force_to_goal= [a_force*math.cos(a_to_goal), a_force*math.sin(a_to_goal)]    
    #print "force to goal", force_to_goal[0], force_to_goal[1]
    #print "A to goal", a_to_goal
    #########################
    # LAB 2 PART A : END
    #########################
...
def obstacle_force():  
   ...
   # print "cur angle", cur_angle
    ...
    #print "scan angle max ", laser_scan.angle_max
    ...
    for i in range(len(laser_scan.ranges)):
        ...
        #sprint "laser scan", laser_scan.ranges[i]
        #########################
        # LAB 2 PART B : BEGIN
        #########################
        # PART B CODE HERE: 
        ...
        #twist.angular.z= .5*strength
        print "strength", strength
        strength= strength*10

        if (strength > 0 ) and (cur_angle < .90) and (cur_angle > -.90):
            #print "force vec", laser_scan.angle_max, laser_scan.angle_min
            force_vector = [strength*math.cos(laser_scan.angle_increment), strength*math.sin(laser_scan.angle_increment)]
        else:
            force_vector =[0,0]

        
      #  print "cur angle, ", force
        
        #    2. Add this force vector to the 'force_from_obstacles' vector
        force_from_obstacles=add_forces(force_vector, force_from_obstacles)
        #########################
        # LAB 2 PART B : END

        cur_angle = cur_angle + laser_scan.angle_increment
    #print "force from obstacles", force_from_obstacles 
    if force_vector[0] > 0:
        force_from_obstacles[0]=abs(force_from_obstacles[0])
    if force_vector[1] > 0:
        force_from_obstacles[1]=abs([force_from_obstacles[1]])
    print " force from obstacles", force_from_obstacles
...
def get_pf_magnitude_linear(distance):
    ...
    if distance <= distance_threshold:
        ...
        strength= max_strength * (distance_threshold-distance) +3
       # print "strength", strength
        #strength=0
        return strength
